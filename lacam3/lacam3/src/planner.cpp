...
Solution Planner::solve()
{
  ...
  while (!OPEN.empty() && !is_expired(deadline)) {
    ...
    if (H_goal == nullptr && is_same_config(H->C, ins->goals)) {
      ...
      H_goal = H;
      auto initial_plan = backtrack(H_goal);
      improvement_times_ms.push_back(
          static_cast<int64_t>(time_initial_solution));
      improvement_costs.push_back(
          static_cast<int64_t>(get_sum_of_costs(initial_plan)));
...
void Planner::rewrite(HNode *H_from, HNode *H_to)
{
  ...
  while (!Q.empty()) {
    ...
    for (auto n_to : n_from->neighbor) {
      ...
      if (g_val < n_to->g) {
        bool was_h_goal_update = (n_to == H_goal);
        if (was_h_goal_update)
        ...
        if (was_h_goal_update) {
          // Capture (elapsed_ms, sum_of_costs) of the current best solution.
          // We use get_sum_of_costs on the backtracked path so the metric
          // matches downstream lacam0_rs::metrics::get_sum_of_costs (the
          // formula is identical: arrival-time-of-each-agent).
          auto plan = backtrack(H_goal);
          improvement_times_ms.push_back(
              static_cast<int64_t>(elapsed_ms(deadline)));
          improvement_costs.push_back(
              static_cast<int64_t>(get_sum_of_costs(plan)));
        }
      }
    }
  }
